...
- The confirmed event facts already support the existing December calendar and start-only iCalendar action. A separate Pastel `예식 안내` block or countdown is an optional presentation decision, not a missing fact; do not add a countdown, event timeline, or duplicate calendar control until the user chooses that presentation.
