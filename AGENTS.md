...
- Pastel displays the exact English label `Our Wedding Day` as a self-hosted handwritten/calligraphic webfont in a lightly diagonal two-line `Our Wedding` / `Day` treatment. Let it extend slightly past the inset photo's left edge without clipping, adding a card, or changing the approved crop; reduced motion keeps the static diagonal treatment.
- Pastel carries a subtle paper texture and distributed powder-blue/blush watercolor wash across the full invitation, rather than concentrating color only in the hero or top-right. Keep it behind readable content and use no separate color panels.
