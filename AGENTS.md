...
- Confirmed family display is `김웅기 · 홍정화의 아들 김종인` and `유효상 · 정소은의 딸 유지혜`; birth order such as 장남·장녀 remains unknown and must not be inferred. The confirmed couple and parent phone numbers are public invitation content and must be presented through accessible call and text actions without duplicating the literal values in tests or logs.
- Keep contact details in collapsed groom-side and bride-side disclosures near the venue information, with the bottom `연락하기` action scrolling to that shared section. A collapsed static-client disclosure is visual organization, not access control.
- Account guidance remains absent until bank, account number, holder, side, and public-scope approval are all confirmed.
