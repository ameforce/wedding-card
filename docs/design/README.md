...
현재 확인되지 않은 공개 콘텐츠는 없다.

RSVP는 사용하지 않는 것으로 확정했으며 공개 UI, API, 저장소, 관리자 화면에 추가하지 않는다.

검색엔진 노출은 비활성화로 확정되었으며, 대표 URL은 `https://wdcard.enmsoftware.com/`이다.

모든 공개 콘텐츠가 확정되어 `scripts/check-content.mjs`와 기본 production build가 통과한다.
...
- `og/wedding-card-1200x630.jpg`: 승인된 `KSJ_1400.jpg` 원본을 AI 편집 없이 중앙 크롭·JPEG 최적화한 공유 미리보기 자산.
...
- 카운트다운, 자동재생 음악
- 사용하지 않기로 확정한 RSVP
- 확정된 OG 공유 미리보기는 `KSJ_1400.jpg`의 1200×630 단순 크롭·최적화 파생본을 사용한다.

카운트다운과 자동재생 음악은 별도 결정 없이는 추가하지 않으며, RSVP는 사용 결정이 명시적으로 변경되기 전까지 추가하지 않는다.
