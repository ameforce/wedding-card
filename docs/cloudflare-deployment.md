...
- The production render canary must independently probe HTTP `GET` and `HEAD` on a sentinel path with an encoded query, with redirects disabled, and require the server's exact HTTPS `Location` and deployed Worker identity. HTTP has its own bounded identity convergence: retry only an old or missing identity; a matching identity with an incorrect redirect fails immediately. Preserve status, Location, tag, and version observations on failure. A browser's internal `307 HttpsUpgrades` is not server redirect evidence. Verify the final HTTPS document, revision, hero, and errors after entry; retain the normal HTTPS warm/cold scenarios.
