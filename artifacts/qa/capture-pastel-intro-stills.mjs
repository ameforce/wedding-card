...
await videoPage.waitForTimeout(5300);
await videoContext.close();

// 2) 390px 플립북 구간별 스틸 (F0 홀드 → 크로스페이드 → 낙하 → 개봉)
...
await shotPage.screenshot({ path: "artifacts/qa/intro-2-frame-f1.png" });
await shotPage.waitForTimeout(500);
await shotPage.screenshot({ path: "artifacts/qa/intro-3-frame-f2.png" });
await shotPage.waitForTimeout(550);
await shotPage.screenshot({ path: "artifacts/qa/intro-4-frame-f3.png" });
await shotPage.waitForTimeout(600);
...
await narrowPage.waitForTimeout(2750);
...
await widePage.waitForTimeout(2750);
